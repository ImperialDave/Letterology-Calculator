...
const INTRO = [
  "Calculara belonged to the letters. Then G opened the ports and let the numbers in.",
  "You are a lowercase c. Nobody counted you. A piece of old type stuck in your curve and woke you up.",
  "Find the Drop Cap. Free the other letters. Stop G before the whole planet is rounded down.",
];
  ...
  return (
    <div className="rounded-lg border border-border bg-elevated/90 p-3 text-left">
      ...
      <div className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 text-sm text-muted">
        ...
        <span>Stem wall · hold down + L (or Shelf on touch) for a platform</span>
      ...
      <p className="mt-3 text-[11px] leading-snug text-subtle">
        Touch: left half is a floating stick. Jump and Fang under the right thumb. Stem builds a wall; Shelf builds a platform — no need to hold down. Talk only shows when something is in reach.
      </p>
    </div>
  );
}
      ...
      {ui.mode === "dead" && (
        <div className="absolute inset-0 flex items-center justify-center bg-bg/75 px-6">
          <div className="max-w-sm text-center">
            <h2 className="font-display text-5xl">You went down</h2>
            <p className="mt-3 text-muted">A number got the better of you.</p>
            <button
              type="button"
      ...
      {ui.mode === "win" && (
        <div className="absolute inset-0 flex items-center justify-center bg-bg/80 px-6">
          <div className="max-w-md text-center">
            <p className="text-sm uppercase tracking-[0.25em] text-accent">G is down</p>
            <h2 className="mt-2 font-display text-5xl">One traitor stopped</h2>
            <p className="mt-4 text-muted leading-relaxed">
              G opened the ports. The numbers still hold most of Calculara. You have ink left. The work is not over.
            </p>
            <button
              type="button"
              className="mt-6 h-12 w-full rounded-lg bg-fg text-bg"
...
function MobilePad({
  ...
  return (
    <div className="gb-pad pointer-events-none absolute inset-0 z-20">
      ...
      <div
        className="pointer-events-none absolute bottom-0 right-0 h-[14rem] w-[12.2rem]"
      ...
      >
        ...
        <button
          type="button"
          data-role="word"
          className="gb-pad-btn pointer-events-auto absolute bottom-[5.55rem] right-[5.65rem] flex h-[2.85rem] w-[2.85rem] items-center justify-center rounded-full border border-accent/35 bg-surface/55 text-[10px] font-medium tracking-wide text-accent"
          aria-label="Scribe stem"
        >
          Stem
        </button>
        <button
          type="button"
          data-role="shelf"
          className="gb-pad-btn pointer-events-auto absolute bottom-[8.35rem] right-[5.65rem] flex h-[2.85rem] w-[2.85rem] items-center justify-center rounded-full border border-accent/45 bg-accent-dim text-[10px] font-medium tracking-wide text-accent"
          aria-label="Scribe shelf"
        >
          Shelf
        </button>
        <button
