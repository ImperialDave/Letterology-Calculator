...
function Hud({ finger }: { finger: boolean }) {
  ...
  return (
    <div className="pointer-events-none absolute inset-0 z-10 flex flex-col justify-between p-3 pt-[max(0.75rem,env(safe-area-inset-top))] pr-[max(0.75rem,env(safe-area-inset-right))] pb-3 pl-[max(0.75rem,env(safe-area-inset-left))]">
      ...
      <div className={`flex items-end justify-between gap-3 ${finger ? "mb-44 short:mb-24" : "mb-0"}`}>
  ...
  return (
    <Modal onClose={() => getGame()?.setPhase("title")}>
      ...
      <ul className="mt-5 space-y-2 text-sm text-fg">
        ...
        <li>
          <strong className="font-medium">Phone</strong> — left stick to drive and drill, including
          diagonals. Right cluster: charge, spare can, patch, recall, and shop on the pad.
        </li>
        <li>
...
function TouchPad({ finger }: { finger: boolean }) {
  ...
  return (
    <div className="pointer-events-none absolute inset-0 z-20">
      <Stick />
      <div className="pointer-events-auto absolute right-[max(0.75rem,env(safe-area-inset-right))] bottom-[max(0.7rem,env(safe-area-inset-bottom))] flex flex-col items-end gap-1.5 short:flex-row short:items-end">
...
function Stick() {
  ...
  return (
    <div
      ref={ref}
      className="pointer-events-auto absolute left-[max(0.75rem,env(safe-area-inset-left))] bottom-[max(0.7rem,env(safe-area-inset-bottom))] size-[8.25rem] touch-none rounded-full border border-fg/20 bg-surface/75 shadow-panel short:size-24"
      onPointerDown={(e) => {
        e.preventDefault();
        e.stopPropagation();
    ...
    >
      <div
        className="absolute size-14 rounded-full border border-fg/30 bg-elevated shadow-panel short:size-10"
        style={{
          left: `${50 + knob.x * 28}%`,
          top: `${50 + knob.y * 28}%`,
